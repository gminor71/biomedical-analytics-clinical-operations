...
# QC: ensure slots exist
qc_assert(exists("NARRATIVE_SLOTS"), "NARRATIVE_SLOTS not found. Did you create src/narrative_slots.R?")

# Optional but strongly recommended: validate required keys
validate_narrative_slots(
  NARRATIVE_SLOTS,
  required = c("study_id", "study_title", "covariates_text", "contrast_label")
)
...
# ---- Identify model type and extract primary estimand (slot-driven) ----
effect_line <- NULL
effect_sentence <- NULL
diag_sentence <- NULL

contrast_label <- if (!is.null(NARRATIVE_SLOTS$contrast_label)) NARRATIVE_SLOTS$contrast_label else "Active vs Control"
cov_txt <- if (!is.null(NARRATIVE_SLOTS$covariates_text)) NARRATIVE_SLOTS$covariates_text else "age, sex, and baseline severity"

# Default term names (can be overridden per project in narrative_slots.R)
term_trt <- if (!is.null(NARRATIVE_SLOTS$term_trt))
  NARRATIVE_SLOTS$term_trt else "trtActive"

term_slope <- if (!is.null(NARRATIVE_SLOTS$term_slope))
  NARRATIVE_SLOTS$term_slope else "trtActive:time30"

slope_unit <- if (!is.null(NARRATIVE_SLOTS$slope_unit))
  NARRATIVE_SLOTS$slope_unit else "per 30 days"

if (inherits(primary_model, "glm")) {
  # Logistic regression (CT01-like)
  tab <- summary(primary_model)$coefficients
  qc_assert(term_trt %in% rownames(tab), paste0("Primary glm missing ", term_trt, "."))
  
  beta <- tab[term_trt, "Estimate"]
  se   <- tab[term_trt, "Std. Error"]
  p    <- tab[term_trt, "Pr(>|z|)"]
  or   <- exp(beta)
  ci   <- exp(beta + c(-1, 1) * 1.96 * se)
  
  effect_line <- paste0(
    "Adjusted OR (", contrast_label, "): ",
    fmt_ci(or, ci[1], ci[2]),
    "; p=", fmt_p(p), "."
  )
  effect_sentence <- sent_primary_or(or, ci[1], ci[2], p, covariates = cov_txt)
  
  diag_sentence <- if (!is.null(NARRATIVE_SLOTS$diag_sentence_glm)) {
    NARRATIVE_SLOTS$diag_sentence_glm
  } else {
  ...
  qc_assert(term_trt %in% names(b), paste0("Primary coxph missing ", term_trt, "."))
  
  beta <- as.numeric(b[term_trt])
  se   <- as.numeric(sqrt(V[term_trt, term_trt]))
  z    <- beta / se
  ...
  effect_line <- paste0(
    "Adjusted HR (", contrast_label, "): ",
    fmt_ci(hr, ci[1], ci[2]),
    "; p=", fmt_p(p), "."
  )
  effect_sentence <- sent_primary_hr(hr, ci[1], ci[2], p, covariates = cov_txt)
  
  # PH diagnostics from frozen CSV (slot-controlled)
  ...
  } else if (isTRUE(NARRATIVE_SLOTS$supports_ph_diag)) {
    diag_sentence <- sent_diagnostics_ph(NA_real_)
  } else {
    diag_sentence <- if (!is.null(NARRATIVE_SLOTS$diag_sentence_generic)) {
      NARRATIVE_SLOTS$diag_sentence_generic
    } else {
      "Model diagnostics and assumptions were reviewed."
    }
  }
  
} else if (inherits(primary_model, "lmerMod")) {
  # Mixed model (CT03-like)
  b <- lme4::fixef(primary_model)
  V <- as.matrix(vcov(primary_model))
  qc_assert(term_slope %in% names(b), paste0("Primary mixed model missing ", term_slope, "."))
  
  beta <- as.numeric(b[term_slope])
  se   <- as.numeric(sqrt(V[term_slope, term_slope]))
  z    <- beta / se
  p    <- 2 * stats::pnorm(abs(z), lower.tail = FALSE)
  
  effect_line <- paste0(
    "Treatment-by-time interaction (", slope_unit, "): beta=",
    fmt_num(beta, 4),
    " (SE=", fmt_num(se, 4), "); p=", fmt_p(p), "."
  )
  effect_sentence <- sent_primary_lme_slope(beta, se, p, unit = slope_unit)
  
  diag_sentence <- if (!is.null(NARRATIVE_SLOTS$diag_sentence_lme)) {
    NARRATIVE_SLOTS$diag_sentence_lme
  } else {
    "Model convergence and fit diagnostics were reviewed; the mixed model uses maximum likelihood under a MAR assumption."
  }
...
# ---- Optional: CT03 emmeans sentence (slot-controlled) ----
emm_sentence <- NULL
