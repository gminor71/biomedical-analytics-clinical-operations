# ct00_design/01_simulate_trial.R
# Purpose:
# Simulate a simple two-arm randomized clinical trial with a binary endpoint.
# Example endpoint: response / non-response, event / no event.
#
# Outputs:
# - results/ct00_design/simulation_results.csv
# - results/ct00_design/simulation_summary.csv

rm(list = ls())

source("ct00_design/00_design_parameters.R")
...
simulate_one_trial <- function(n_per_arm, p_control, p_treatment, alpha = 0.05) {
  ...
  # prop.test provides approximate CI and p-value
  test <- prop.test(
  ...
  # Wald OR estimate via logistic regression
  dat <- data.frame(
...
# Save outputs
# ---------------------------
out_dir <- file.path("results", "ct00_design")
